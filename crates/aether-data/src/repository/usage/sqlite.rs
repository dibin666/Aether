...
"#,
    ...
    async fn summarize_provider_api_key_consumption(
        ...
        let created_until_unix_secs = query
            ...
            .map(|value| to_i64(value, "usage.created_until_unix_secs"))
            .transpose()?;
        let mut builder = QueryBuilder::<Sqlite>::new(format!(
            r#"
WITH key_bounds(provider_api_key_id, created_from_unix_secs) AS (
VALUES
"#,
        ));
        {
            let mut separated = builder.separated(", ");
            for (provider_api_key_id, created_from_unix_secs) in
                &query.created_from_unix_secs_by_provider_api_key_id
            {
                separated.push("(");
                separated.push_bind(provider_api_key_id.clone());
                separated.push(", ");
                separated.push_bind(to_i64(
                    *created_from_unix_secs,
                    "usage.created_from_unix_secs",
                )?);
                separated.push(")");
            }
        }
...
SELECT
  ...
  COUNT(*) AS request_count,
  COALESCE(SUM(MAX(COALESCE("usage".input_tokens, 0), 0)), 0) AS input_tokens,
  COALESCE(SUM(MAX(COALESCE("usage".output_tokens, 0), 0)), 0) AS output_tokens,
  COALESCE(SUM({cache_creation_expr}), 0) AS cache_creation_tokens,
  ...
  COALESCE(SUM(COALESCE(CAST("usage".total_cost_usd AS REAL), 0)), 0)
    AS total_cost_usd
FROM "usage"
...
"#,
        ...
        builder.push_bind(query.provider_id.clone());
        builder.push(
            r#"
  AND "usage".created_at_unix_ms >= key_bounds.created_from_unix_secs
  AND (
"#,
        );
        builder.push_bind(created_until_unix_secs);
        builder.push(r#" IS NULL OR "usage".created_at_unix_ms < "#);
        builder.push_bind(created_until_unix_secs);
        builder.push(
            r#"
  )
...
"#,
        ...
        for row in rows {
            let provider_api_key_id: String = row.try_get("provider_api_key_id").map_sql_err()?;
            let total_cost_usd = sqlite_real(&row, "total_cost_usd")?;
