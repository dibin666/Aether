    ...
    ($repository:ty) => {
        ...
        impl $crate::repository::usage::UsageReadRepository for $repository {
            ...
            async fn summarize_provider_api_key_consumption(
                &self,
                query: &$crate::repository::usage::ProviderApiKeyConsumptionSummaryQuery,
            ) -> Result<
                std::collections::BTreeMap<
                    String,
                    $crate::repository::usage::StoredProviderApiKeyConsumptionSummary,
                >,
                $crate::DataLayerError,
            > {
                let repository = self.materialize_read_model().await?;
...
pub(crate) use aether_data_contracts::repository::usage::{
    ...
    ProviderApiKeyWindowUsageRequest, ProxyNodeCounterDelta,
