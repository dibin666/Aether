        ...
        while let Some(row) = rows.try_next().await.map_postgres_err()? {
            ...
            summaries.push(StoredProviderApiKeyWindowUsageSummary {
                ...
                input_tokens: row
                    ...
                    .map_err(|_| {
                        DataLayerError::UnexpectedValue(
                            "usage.input_tokens window aggregate is negative".to_string(),
                        )
                    })?,
                output_tokens: row
                    .try_get::<i64, _>("output_tokens")
                    ...
                    .map_err(|_| {
                        DataLayerError::UnexpectedValue(
                            "usage.output_tokens window aggregate is negative".to_string(),
                        )
                    })?,
                cache_creation_tokens: row
                    .try_get::<i64, _>("cache_creation_tokens")
                    ...
                    .map_err(|_| {
                        DataLayerError::UnexpectedValue(
                            "usage.cache_creation_tokens window aggregate is negative".to_string(),
                        )
                    })?,
                cache_read_tokens: row
                    .try_get::<i64, _>("cache_read_tokens")
