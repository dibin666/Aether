...
SELECT
  ...
  COALESCE(
    SUM(MAX(COALESCE(settlement.billing_output_tokens, "usage".output_tokens, 0), 0)),
    0
  ) AS output_tokens,
  COALESCE(
    SUM(MAX(COALESCE(
  ...
  COALESCE(
    SUM(COALESCE(
      CAST(settlement.billing_total_cost_usd AS REAL),
      CAST("usage".total_cost_usd AS REAL),
      0
    )),
    0
  ) AS total_cost_usd,
