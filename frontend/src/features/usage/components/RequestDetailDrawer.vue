...
// 自助详情的正文随 detail 内联返回，转成与 body_format=raw 相同的 JSON 文本字节，
// 好让 BodyDocument 用同一条解码路径处理。
function encodeInlineBodyBytes(value: unknown): ArrayBuffer {
...
async function ensureBodyContentLoaded() {
  ...
  try {
    // admin 域走上游的按字段惰性取回；自助域的 /api/users/me/usage/{id} 不支持
    // body_field/body_format=raw，正文随 detail 内联返回，这里直接包成同样的文档。
    const inlineBody = detail.value[field]
    const response = isAdminDetail.value
      ? await dashboardApi.getRequestBody(detail.value.id, field, controller.signal)
      : { bytes: encodeInlineBodyBytes(inlineBody), encoding: 'json' as const }
