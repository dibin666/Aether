    ...
    <div class="max-h-[calc(100dvh-13rem)] space-y-4 overflow-y-auto overscroll-contain pr-1 sm:max-h-[min(72vh,44rem)] sm:pr-2">
      <div class="space-y-3 rounded-lg border bg-muted/20 px-3 py-2.5">
        ...
        <div class="space-y-3 rounded-md border bg-background/80 px-3 py-3">
          <div class="grid gap-2 sm:grid-cols-[minmax(0,1fr)_11rem_auto]">
            <Input
            ...
            <Select v-model="statusFilter">
              <SelectTrigger
                ...
                data-testid="pool-quick-action-status-filter"
              >
                <SelectValue placeholder="区分账号状态" />
              </SelectTrigger>
              <SelectContent>
                <SelectItem
...
type StatusFilterValue = NonNullable<PoolKeysQuery['status']>
...
const QUICK_SELECT_OPTIONS: Array<{ value: QuickSelectorValue; label: string }> = [
  ...
  { value: 'oauth_invalid', label: 'Token 异常（含 401）' },
...
const STATUS_FILTER_OPTIONS: Array<{ value: StatusFilterValue; label: string }> = [
  ...
  { value: 'invalid', label: '401 / Token 失效' },
  { value: 'expired', label: 'Token 已过期' },
...
const hasActiveFilters = computed(() => (
  ...
  || activeQuickSelectors.value.length > 0
))
...
function getStatusBadgeLabel(key: PoolKeyDetail): string | null {
  ...
  const oauthReason = String(
    key.status_snapshot?.oauth?.reason
    || key.oauth_invalid_reason
    || oauth?.invalidReason
    || '',
  )
  if (
    (oauth?.isInvalid || oauth?.requiresReauth)
    && /(\b401\b|unauthori[sz]ed|auth(?:entication)?[_ -]?failed)/i.test(oauthReason)
  ) {
    return '401 认证失败'
  }
...
function buildSelectionFilters(): { search?: string; status?: StatusFilterValue; quick_selectors?: string[] } {
...
async function loadKeysPage(): Promise<void> {
  ...
  try {
    ...
    if (requestId === loadRequestId) {
      ...
      console.info('[PoolAccountQuickActionsDialog] loadKeysPage timing', {
...
async function executeAction(actionOverride?: BatchActionValue): Promise<void> {
  ...
  try {
      ...
      const runNext = async (): Promise<void> => {
        while (cursor < exportableKeys.length) {
          ...
          try {
            ...
            failedCount += 1
            // eslint-disable-next-line no-console
            console.error(`[PoolAccountQuickActionsDialog] export failed (${key.key_id}):`, err)
          } finally {
    ...
    // eslint-disable-next-line no-console
    console.info('[PoolAccountQuickActionsDialog] executeAction timing', {
      providerId: props.providerId,
