<template>
  <div class="space-y-6 pb-8">
    ...
    <!-- Dialogs -->
    <PoolRefreshWorkerDialog
      v-model="showRefreshWorkerDialog"
    />
    <OAuthAccountDialog
...
import PoolRefreshWorkerDialog from '@/features/pool/components/PoolRefreshWorkerDialog.vue'
...
function openRefreshWorkerDialog() {
...
async function setProviderProxy(nodeId: string): Promise<void> {
  ...
  try {
    ...
    if (selectedProviderId.value === providerId) selectedProviderData.value = updated
...
function getProviderToggleButtonTitle(): string {
  return selectedProviderData.value?.is_active === false
    ? '当前状态：已禁用，点击启用提供商'
    : '当前状态：已启用，点击禁用提供商'
}
