...
fn pool_catalog_key_quota_exhausted(
    ...
    let quota_window_exhausted = quota_snapshot
        .and_then(|quota| pool_quota_snapshot_exhausted(quota, provider_type))
        .unwrap_or(false);
    if quota_window_exhausted {
        return true;
    }
...
mod tests {
    use super::{
        ...
        normalize_pool_plan_type, pool_config_for_candidate,
        pool_key_order_requires_exclusive_lease, pool_quota_snapshot_exhausted,
        PoolCatalogKeyContext, PoolKeyCursor,
    };
    use crate::ai_serving::planner::candidate_resolution::{
