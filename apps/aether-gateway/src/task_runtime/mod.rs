...
fn build_worker_boot_run_id(task_key: &str) -> String {
    compact_background_task_run_id(format!("boot:{task_key}"))
}

fn build_worker_execution_run_id(task_key: &str, instance_id: &str) -> String {
    compact_background_task_run_id(format!("run:{task_key}:{instance_id}"))
}
pub(crate) async fn ensure_worker_execution_run(app: &AppState, task_key: &str) -> Option<String> {
    if !app.has_background_task_data_writer() {
        return None;
    }
    let definition = task_definition(task_key)?;
    let run_id = build_worker_execution_run_id(task_key, app.tunnel.local_instance_id());
    if app
        ...
        .is_none()
    {
        ...
        let run = UpsertBackgroundTaskRun {
            ...
            progress_percent: 0,
            progress_message: Some("worker execution running".to_string()),
            payload_json: None,
            result_json: None,
            error_message: None,
