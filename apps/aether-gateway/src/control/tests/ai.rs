...
#[test]
fn classifies_openai_transcription_as_independent_execution_endpoint() {
    let headers = headers(&[("authorization", "Bearer sk-test")]);
    let uri: Uri = "/v1/audio/transcriptions"
        .parse()
        .expect("uri should parse");
    let decision =
        classify_control_route(&http::Method::POST, &uri, &headers).expect("route should classify");
    ...
    assert_eq!(decision.route_family.as_deref(), Some("openai"));
    assert_eq!(decision.route_kind.as_deref(), Some("transcription"));
    assert_eq!(
        decision.auth_endpoint_signature.as_deref(),
        Some("openai:transcription")
    );
    assert!(decision.is_execution_runtime_candidate());
    assert!(classify_control_route(&http::Method::GET, &uri, &headers).is_none());
}
