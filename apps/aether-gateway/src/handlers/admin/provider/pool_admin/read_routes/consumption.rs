...
use aether_data_contracts::repository::{
    ...
    usage::{ProviderApiKeyWindowUsageRequest, StoredProviderApiKeyWindowUsageSummary},
};
use axum::{
    body::Body,
    http,
...
async fn build_pool_consumption_periods(
    ...
    for period in resolve_pool_consumption_period_defs(tz_offset_minutes) {
        ...
        let (start_unix_secs, end_unix_secs) = bounds.unwrap_or_else(|| {
            let now = chrono::Utc::now().timestamp().max(0) as u64;
            (0, now.saturating_add(1))
        });
        let requests = keys
            .iter()
        ...
        let aggregates = state
            ...
            .await?
            .into_iter()
            .map(|summary| (summary.provider_api_key_id.clone(), summary))
...
fn build_pool_consumption_accounts(
    keys: &[StoredProviderCatalogKey],
    provider_type: &str,
    aggregates: &BTreeMap<String, StoredProviderApiKeyWindowUsageSummary>,
