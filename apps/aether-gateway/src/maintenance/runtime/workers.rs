...
use super::{
    ...
    duration_until_next_stats_hourly_aggregation_run, maintenance_timezone,
    oauth_token_refresh_interval, parse_hhmm_time, perform_oauth_token_refresh_once,
...
pub(crate) fn spawn_oauth_token_refresh_worker(
    ...
    Some(crate::task_runtime::spawn_singleton_worker(
        ...
        |state| async move {
            ...
            let mut deferred_since = None;
            loop {
                tokio::time::sleep(oauth_token_refresh_interval(&state).await).await;
                if should_defer_for_database_pressure(
                    &state.data,
                    "oauth_token_refresh",
                    &mut deferred_since,
