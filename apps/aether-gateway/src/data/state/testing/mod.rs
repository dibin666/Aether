...
use aether_data_contracts::repository::provider_key_task_events::{
    ProviderKeyTaskEventReadRepository, ProviderKeyTaskEventRepository,
...
impl GatewayDataState {
    ...
    #[cfg(test)]
    pub(crate) fn with_provider_key_task_event_repository_for_tests<T>(
        mut self,
        repository: Arc<T>,
    ) -> Self
    where
        T: ProviderKeyTaskEventReadRepository + ProviderKeyTaskEventWriteRepository + 'static,
    {
        let provider_key_task_event_reader: Arc<dyn ProviderKeyTaskEventReadRepository> =
            repository.clone();
        let provider_key_task_event_writer: Arc<dyn ProviderKeyTaskEventWriteRepository> =
            repository;
        self.provider_key_task_event_reader = Some(provider_key_task_event_reader);
        self.provider_key_task_event_writer = Some(provider_key_task_event_writer);
        self
    }
    ...
    #[cfg(test)]
    pub(crate) fn with_provider_key_task_event_standalone_for_tests<T>(repository: Arc<T>) -> Self
    where
        T: ProviderKeyTaskEventRepository + 'static,
    {
        let provider_key_task_event_reader: Arc<dyn ProviderKeyTaskEventReadRepository> =
            repository.clone();
        let provider_key_task_event_writer: Arc<dyn ProviderKeyTaskEventWriteRepository> =
        ...
        Self {
            ...
            provider_quota_reader: None,
            provider_quota_writer: None,
            provider_key_task_event_reader: Some(provider_key_task_event_reader),
            provider_key_task_event_writer: Some(provider_key_task_event_writer),
            routing_group_reader: None,
            routing_group_writer: None,
            usage_reader: None,
            usage_writer: None,
